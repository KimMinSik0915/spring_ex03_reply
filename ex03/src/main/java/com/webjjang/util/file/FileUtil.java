...
public class FileUtil {
	...
	// 중복된 파일의 처리 : 중복이 되지 않는 File 객체를 return
	public static File noDuplicate(String fileFullName) throws Exception {
		...
		String fileName = fileFullName.substring(0, dotPos);	//image.JPG의 File Name = image -> ext : .JPG
		
		String ext = fileFullName.substring(dotPos);
		
		int cnt = 0;		// 중복이 되었을 때 cnt를 중간에 넣는다. : fileName + cnt + ext -> image1.JPG || 중복이 되면 cnt++
		
		// File 정보를 확인
		System.out.println("FileUtil.noDuplicate [fileName] : " + fileName);
		System.out.println("FileUtil.noDuplicate [ext] : " + ext);
		...
		while(true) {
			...
			if (!exist(file)) {		// 중복이 되지 않으면 빠져 나간다.
